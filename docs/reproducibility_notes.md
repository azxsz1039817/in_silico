...
The correct/wrong/no-effect forecast coding uses manuscript-level ground-truth decisions. Most rows can be inferred from `reverse_code`, but school/business open-vs-closed variants require keyword-level handling. The final consistency check matches the manuscript count of 109 correct, 12 wrong, and 58 no-effect prompt forecasts.
